...
auto shiftChars(char* ptr, const size_t start, const size_t end) noexcept {
  ...
  memset(ptr + end, '\0', 4096 - (end - start));
}
...
auto Session::sendRequest(const std::string method, const uint8_t *data, const size_t len) noexcept -> std::variant<Response, int> {
  ...
  auto buffer = std::unique_ptr<char[]>(new (std::nothrow) char[4097]);
  ...
  while (true) {
    ...
    if (isPayload && size > 0) {
      ...
      // This is dumb
      //responsePayload.reserve(responsePayload.size() + size);
      //for (auto *ptr = buffer.get(); ptr < buffer.get() + size; ++ptr) {
        //responsePayload.push_back(static_cast<uint8_t>(*buffer.get()));
      //}
